...

	{
		...
		while ( a != -1 ) // do the body if "a" is not end-of-stream
		{
			...
			if ( nextSt == State.UNDEF ) // The FA will halt.
			{
				if ( state.isFinal() )
				{
					if( state.compareTo(State.Id) == 0 ) //Check if token is accepted as identifier
					{
						switch (t) //Check if accepted identifier token is a class, if, or null keyword
						{
							case "class":
								state = State.Class; //Change DFA to corresponding state.
								break;
							case "if":
								state = State.If;
								break;
